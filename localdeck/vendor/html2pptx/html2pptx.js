...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.classList?.contains('placeholder') && el.tagName !== 'TABLE') {
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        const hasBlockTextElements = liElements.some((li) => {
          ...
          return Array.from(inlineElements).some((el) => {
            if (
              el.classList?.contains('__pseudo_before__') ||
              el.classList?.contains('__pseudo_after__')
            ) return false;
            const computed = window.getComputedStyle(el);
            if (isBulletMarker(el, computed)) return false;
            const display = computed.display;
      ...
      if (el.tagName === 'LI') {
        ...
        const hasBlockInline = Array.from(inlineElements).some((child) => {
          if (
            child.classList?.contains('__pseudo_before__') ||
            child.classList?.contains('__pseudo_after__')
          ) return false;
